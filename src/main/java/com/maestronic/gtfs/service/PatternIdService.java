    ...
    public Integer parseSaveData(String path) {
        ...
        try (BufferedReader fileReader = new BufferedReader(new FileReader(path));
            ...
            for (CSVRecord csvRecord : csvParser) {
                PatternId patternId = new PatternId(
                        csvRecord.get("trip_id"),
                        csvRecord.get("pattern_id"),
                        csvRecord.get("pattern_name")
                );

                session.saveOrUpdate(patternId);
            ...
            session.flush();
            session.clear();
            Logger.info("Parse and save " + GlobalVariable.PATTERN_ID + " file is complete.");
            return dataCount;
        } catch (Exception e) {
            String logMessage = "Fail to parse and save " + GlobalVariable.PATTERN_ID + " file. " + e.getMessage();
